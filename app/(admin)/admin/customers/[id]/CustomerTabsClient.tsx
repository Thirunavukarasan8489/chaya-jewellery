        ...
        {activeTab === "business" && (
          <div className="space-y-6">
            ...
            {customer.type === "BUSINESS" ? (
                ...
                {customer.businessDetails?.gstRegistered && (
                  <>
                    ...
                    <div>
                      ...
                      <p className="font-medium text-plum-900 dark:text-white">
                        {customer.businessDetails?.legalName || "-"}
                      </p>
                    </div>
                  </>
                )}
              ...
              <p className="text-sm text-plum-500 dark:text-plum-400">
                This is a personal customer account. Business information is not
                applicable.
              </p>
            )}
          </div>
            ...
            {customer.addresses && customer.addresses.length > 0 ? (
              <div className="grid grid-cols-1 gap-4">
                {customer.addresses.map((addr: any, idx: number) => (
                  <div
                    key={idx}
                    className="border border-gray-200 dark:border-plum-800 rounded-md p-4 bg-gray-50 dark:bg-plum-950"
                  >
                    <p className="font-medium text-plum-900 dark:text-white">
                      {addr.name}
                    </p>
                  ...
                  <tbody className="divide-y divide-gray-200 dark:divide-plum-800 text-plum-800 dark:text-plum-200">
                    {payments.map((p) => (
                      <tr key={p._id}>
                        ...
                        <td className="px-4 py-3">
                          <span
                            className={`px-2 py-1 text-xs rounded-none ${p.status === "SUCCESS" ? "bg-emerald-100 text-emerald-700" : "bg-red-100 text-red-700"}`}
                          >
                            {p.status}
                          </span>
                        </td>
        ...
        {activeTab === "returns_refunds" && (
          <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div>
                <h4 className="font-medium text-plum-800 dark:text-plum-200 mb-3">
                  Returns
                </h4>
                {returns && returns.length > 0 ? (
                  <ul className="space-y-3">
                    {returns.map((r) => (
                      <li
                        key={r._id}
                        className="p-3 border border-gray-200 dark:border-plum-800 rounded-md"
                      >
                        <p className="text-sm font-medium text-plum-900 dark:text-white">
                          Status: {r.status}
                        </p>
                        <p className="text-xs text-plum-500 dark:text-plum-400 mt-1">
              ...
              {orders &&
                orders.map((order) => (
                  <div key={order._id} className="pl-6 relative">
                    ...
                    <p className="text-sm font-medium text-plum-900 dark:text-white">
                      Placed Order {order.orderNumber}
                    </p>
                    <p className="text-xs text-plum-500 dark:text-plum-400">
