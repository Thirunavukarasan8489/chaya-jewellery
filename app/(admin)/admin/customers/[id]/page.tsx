...
export default async function CustomerDetailsPage({
    ...
    return (
      <div className="space-y-6">
        <Link
          ...
          className="inline-flex items-center text-sm font-medium text-gold-600 hover:text-gold-700 dark:text-gold-400 dark:hover:text-gold-300"
        >
  ...
  if (customer.contact?.email)
    queryConditions.push({ email: customer.contact.email });
  if (customer.contact?.phone)
    queryConditions.push({ phone: customer.contact.phone });
  ...
  if (queryConditions.length > 0) {
    ...
    if (orders.length > 0) {
      ...
      if (ReturnRequest) {
        returns =
          (await ReturnRequest.find({ orderId: { $in: orderIds } })
            .sort({ createdAt: -1 })
            .lean()) || [];
      }
      if (Refund) {
        refunds =
          (await Refund.find({ orderId: { $in: orderIds } })
            .sort({ createdAt: -1 })
            .lean()) || [];
      }
    }
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <Link
          href="/admin/customers"
          className="inline-flex items-center text-sm font-medium text-gold-600 hover:text-gold-700 dark:text-gold-400 dark:hover:text-gold-300 transition-colors"
        >
          <ChevronLeft className="w-4 h-4 mr-1" />
          Back to Customers
        </Link>
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        <div className="flex gap-4">
          <div className="bg-white dark:bg-plum-900 border border-gray-200 dark:border-plum-800 rounded-lg px-4 py-2 text-center shadow-sm">
            <p className="text-xs text-plum-500 dark:text-plum-400 uppercase tracking-wider">
              Total Orders
            </p>
            <p className="text-lg font-semibold text-plum-900 dark:text-white">
              {orders.length}
            </p>
          </div>
          <div className="bg-white dark:bg-plum-900 border border-gray-200 dark:border-plum-800 rounded-lg px-4 py-2 text-center shadow-sm">
