  ...
  if (!mounted) {
    ...
    return (
      <div className="w-full space-y-1.5">
        ...
        {helperText && !error && (
          <p className="text-xs text-gold-400 mt-1">{helperText}</p>
        )}
      </div>
    );
  }
    ...
    <div className="w-full space-y-1.5">
      ...
      <Select
        ...
        classNames={{
          ...
          valueContainer: () =>
            "flex items-center gap-1.5 flex-wrap text-plum-900 dark:text-ivory-100",
          singleValue: () =>
            "text-plum-900 dark:text-ivory-100 text-sm font-normal",
          multiValue: () =>
            "flex items-center bg-plum-100 dark:bg-plum-800 text-plum-900 dark:text-ivory-100 rounded px-2 py-0.5 text-xs font-medium gap-1 border border-plum-200 dark:border-plum-700",
          multiValueLabel: () => "text-plum-900 dark:text-ivory-100",
          multiValueRemove: () =>
            "text-plum-500 hover:text-rose-600 cursor-pointer rounded-sm",
          placeholder: () => "text-plum-400 text-sm",
          input: () => "text-plum-900 dark:text-ivory-100 text-sm",
          menu: () =>
            "mt-1.5 bg-white dark:bg-plum-900 border border-gray-200 dark:border-plum-800 rounded-lg shadow-lg overflow-hidden z-50 py-1",
          ...
          dropdownIndicator: () =>
            "p-1 text-plum-400 hover:text-plum-600 dark:hover:text-plum-200 cursor-pointer",
          clearIndicator: () =>
            "p-1 text-plum-400 hover:text-rose-500 cursor-pointer",
          indicatorSeparator: () =>
            "bg-gray-200 dark:bg-plum-700 my-1 mx-1.5 w-px",
        }}
        {...props}
      />
      {error && <p className="text-xs text-red-500 mt-1">{error}</p>}
