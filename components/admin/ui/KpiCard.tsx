...
export default function KpiCard({
  ...
  const colorMap = {
    ...
    purple:
      "bg-plum-100 text-plum-800 dark:bg-plum-800 dark:text-plum-200 border border-plum-200 dark:border-plum-700",
    amber:
  ...
  return (
    <div
      className={cn(
        "bg-white dark:bg-plum-900 rounded-xl p-6 shadow-xs border border-gray-200 dark:border-plum-800 transition-all hover:border-plum-400/50 hover:shadow-md",
        className,
      )}
    >
      <div className="flex justify-between items-start">
        <div>
          <p className="text-sm font-medium text-plum-600 dark:text-plum-300">
            {title}
          </p>
          <h3 className="text-2xl font-bold text-plum-900 dark:text-ivory-100 mt-2">
            {value}
          </h3>
        </div>
      ...
      {trend && trendValue && (
        <div className="mt-4 flex items-center gap-1.5 text-sm">
          ...
          <span className="text-plum-500 dark:text-plum-400 ml-1">
            vs last month
          </span>
        </div>
      )}
