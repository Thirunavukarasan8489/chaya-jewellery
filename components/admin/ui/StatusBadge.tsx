...
export default function StatusBadge({
  ...
  const variantStyles = {
    ...
    neutral:
      "bg-gold-100 text-gold-700 dark:bg-gold-800 dark:text-gold-300 border border-gold-200 dark:border-gold-700",
  };
  ...
  if (!variant && status) {
    ...
    if (
      ...
      ["PENDING", "LOW_STOCK", "CONTACTED", "FOLLOW_UP", "PROCESSING"].includes(
        s,
      )
    )
    ...
    else if (["NEW", "SHIPPED", "OUT_FOR_DELIVERY"].includes(s))
      activeVariant = "info";
  }
  ...
  return (
    <span
      className={cn(
        "inline-flex items-center px-2.5 py-0.5 rounded-none text-[11px] uppercase tracking-wider font-semibold",
