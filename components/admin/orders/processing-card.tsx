    ...
    return (
      <div className="bg-red-50 border border-red-200 rounded-lg p-6 flex flex-col items-center justify-center text-center">
        ...
        <p className="text-sm text-red-700">
          This order is {currentStatus.toLowerCase()} and cannot be advanced
          further.
        </p>
      </div>
    );
      ...
      <div className="flex items-center justify-between mb-8 overflow-x-auto pb-4">
        {FLOW.map((step, idx) => {
          ...
          return (
            <div
              key={step.status}
              className="flex flex-col items-center relative z-10 flex-shrink-0 mx-2"
            >
              <div
                className={`w-10 h-10 rounded-none flex items-center justify-center transition-colors ${
                  isCompleted
                    ...
                    : isCurrent
                      ? "bg-blue-100 text-blue-700 border-2 border-blue-500 shadow-[0_0_0_4px_rgba(59,130,246,0.1)]"
              ...
              <span
                className={`text-xs mt-2 font-medium ${isCurrent ? "text-blue-700" : isCompleted ? "text-green-700" : "text-gray-500"}`}
              >
                {step.label}
              </span>
            </div>
