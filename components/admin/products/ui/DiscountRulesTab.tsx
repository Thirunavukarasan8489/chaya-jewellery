    ...
    <div className={isActive ? "space-y-5" : "hidden"}>
      <div className="flex items-center justify-between border-b border-gray-200 dark:border-plum-800 pb-3">
        ...
        <AdminButton
          ...
          variant="outline"
          onClick={() =>
            appendRule({ minQty: 1, maxQty: 10, discountPercentage: 5 })
          }
        >
      ...
      <div className="space-y-3">
        {ruleFields.length === 0 && (
          <div className="p-4 border border-gray-200 dark:border-plum-800 border-dashed rounded-xl text-center text-plum-500 text-sm">
            No discount rules applied. Click &quot;Add Rule&quot; to set up tier
            pricing.
          </div>
        )}
        {ruleFields.map((field, index) => (
          <div
            key={field.id}
            className="flex items-start gap-4 p-4 border border-gray-200 dark:border-plum-700 rounded-xl bg-white dark:bg-plum-950 relative"
          >
            <div className="flex-1 grid grid-cols-1 md:grid-cols-3 gap-4">
              ...
              <AdminInput
                ...
                error={
                  errors.discountRules?.[index]?.discountPercentage?.message
                }
              />
            </div>
            <button
