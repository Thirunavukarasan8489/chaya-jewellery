...
export default function ProductForm({
  ...
  const [coverFile, setCoverFile] = useState<{
    ...
    initialData?.primaryImage?.url
      ? { previewUrl: initialData.primaryImage.url, isExisting: true }
      : null,
  );
  ...
  const [validationErrors, setValidationErrors] = useState<
    { field: string; message: string; tabId: string }[] | null
  >(null);
  ...
  const handleGalleryUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    setValue(
      "gallery",
      [...currentGallery, ...newFiles.map(() => ({ url: "", altText: "" }))],
      { shouldValidate: true },
    );
  };
  ...
  const onSubmit = async (data: ProductFormValues) => {
    const uploadAndSubmit = async () => {
      ...
      if (!res.success) {
        throw new Error(
          res.error || `Failed to ${initialData ? "update" : "create"} product`,
        );
      }
      return res;
    };
  ...
  const onInvalid = (formErrors: any) => {
    ...
    const getTabForField = (fieldName: string) => {
      for (const tab of Object.keys(tabFields)) {
        if (
          tabFields[tab].some(
            (f) => fieldName === f || fieldName.startsWith(f + "."),
          )
        )
          return tab;
      }
          ...
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-xs text-plum-500 dark:text-plum-400 mt-0.5">
                Unique slug is automatically generated in the backend from
                product title.
              </p>
            </div>
          </div>
          ...
          <div className="bg-white dark:bg-plum-950 rounded-xl shadow-2xl max-w-md w-full border border-gray-200 dark:border-plum-800 overflow-hidden">
            <div className="p-4 border-b border-gray-200 dark:border-plum-800 flex justify-between items-center bg-rose-50 dark:bg-rose-900/20">
              ...
              <button
                ...
                className="text-plum-400 hover:text-plum-600 dark:hover:text-plum-200"
              >
                <X size={20} />
              </button>
            </div>
            <div className="p-4 max-h-96 overflow-y-auto">
              <p className="text-sm text-plum-600 dark:text-plum-300 mb-4">
                Please fix the following required fields before saving:
              </p>
              <ul className="space-y-3">
                {validationErrors.map((err, i) => (
                  <li
                    key={i}
                    className="flex flex-col text-sm p-3 bg-gray-50 dark:bg-plum-900 rounded-lg border border-gray-100 dark:border-plum-800"
                  >
                    <span className="font-medium text-plum-800 dark:text-plum-200 capitalize">
                      {err.field.replace(/\./g, " ")}
                    </span>
                    <span className="text-rose-600 dark:text-rose-400 mt-1">
                      {err.message}
                    </span>
                    <button
                      type="button"
