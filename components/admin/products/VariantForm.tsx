...
function buildVariantSchema(priceOnValue: boolean) {
  return z
    .object({
      ...
      primaryImage: z
        .object({ url: z.string(), altText: z.string().optional() })
        .optional(),
      gallery: z
...
export default function VariantForm({
  ...
  const [coverFile, setCoverFile] = useState<{
    ...
    variant.primaryImage?.url
      ? { previewUrl: variant.primaryImage.url, isExisting: true }
      : null,
  );

  const [galleryItems, setGalleryItems] = useState<
    { id: string; file?: File; previewUrl: string; isExisting: boolean }[]
  >(
    (variant.gallery || [])
  ...
  const removeGalleryImage = (indexToRemove: number) => {
    ...
    setValue(
      "gallery",
      currentGallery.filter((_, idx) => idx !== indexToRemove),
    );
  };
    ...
    try {
      ...
      if (coverFile && !coverFile.isExisting) {
        ...
        if (res.success && res.data) {
          finalPrimaryImage = {
            url: res.data.secureUrl || res.data.url,
            altText: data.primaryImage?.altText || "",
          };
        } else {
      ...
      const galleryUploadPromises = galleryItems.map(async (item, i) => {
        ...
        if (res.success && res.data) {
          return {
            ...
            data: {
              url: res.data.secureUrl || res.data.url,
              altText: finalGallery[i]?.altText || item.file!.name,
            },
          };
        } else {
      ...
      if (result.success) {
        ...
        toast.error(
          result.error ||
            `Failed to ${isCreateMode ? "create" : "update"} variant`,
        );
      }
    } catch (err: any) {
        ...
        <div className="py-4 px-6 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-3">
            <AdminButton
              type="button"
              variant="outline"
              onClick={() => router.push("/admin/productvarients")}
            >
              Cancel
            </AdminButton>
            <AdminButton
        ...
        <div className="flex-1 w-full bg-white dark:bg-plum-950 border border-gray-200 dark:border-plum-800 rounded-2xl shadow-sm p-6 lg:p-8">
          <FormProvider {...methods}>
            <form onSubmit={handleFormSubmit} className="space-y-6">
              ...
              <div className={activeTab === "basic" ? "space-y-6" : "hidden"}>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <AdminInput
                    ...
                    className={
                      priceOnValue ? "opacity-50 cursor-not-allowed" : ""
                    }
                    {...register("name")}
                ...
                <div className="space-y-3">
                  ...
                  {coverFile ? (
                    ...
                    <div className="border-2 border-dashed border-gray-300 dark:border-plum-700 rounded-xl p-6 text-center bg-gray-50 dark:bg-plum-900/40 hover:bg-gray-100 dark:hover:bg-plum-800/60 transition-colors relative cursor-pointer max-w-md">
                      ...
                      />
                      <p className="text-sm font-medium text-plum-700 dark:text-plum-300">
                ...
                <div className="space-y-4 pt-4 border-t border-gray-100 dark:border-plum-800">
                  ...
                  <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-4">
                    ...
                    <label className="aspect-square rounded-xl border-2 border-dashed border-gray-300 dark:border-plum-700 bg-gray-50 dark:bg-plum-900/40 hover:bg-gray-100 dark:hover:bg-plum-800 flex flex-col items-center justify-center cursor-pointer transition-colors">
                      ...
                      <span className="text-xs font-medium text-gray-600 dark:text-plum-300">
                        Add Images
                      </span>
                    </label>
                  </div>
              ...
              {/* 3. Discount Rules */}
              <div
                className={activeTab === "discount" ? "space-y-6" : "hidden"}
              >
                <div className="flex items-center justify-between border-b border-gray-100 dark:border-plum-800 pb-3">
                  <div>
                    <h3 className="text-lg font-semibold text-plum-900 dark:text-ivory-100">
                      Discount Rules
                    </h3>
                    <p className="text-xs text-plum-500 mt-1">
                      Quantity-based tier discounts for this variant only.
                    </p>
                  </div>
                  <AdminButton
                    type="button"
                ...
                <div className="space-y-3">
                  ...
                  {ruleFields.map((field, index) => (
                    <div
                      key={field.id}
                      className="flex items-start gap-4 p-4 border border-gray-200 dark:border-plum-700 rounded-xl bg-white dark:bg-plum-900 relative"
                    >
                      <div className="flex-1 grid grid-cols-1 md:grid-cols-3 gap-4">
                        ...
                        <AdminInput
                          ...
                          label="Discount (%)"
                          {...register(
                            `discountRules.${index}.discountPercentage`,
                          )}
                          error={
                            errors.discountRules?.[index]?.discountPercentage
                              ?.message
                          }
                        />
                      </div>
              ...
              {/* 4. Purchase Rule */}
              <div
                className={activeTab === "purchase" ? "space-y-5" : "hidden"}
              >
                <h3 className="text-lg font-semibold text-plum-900 dark:text-ivory-100 flex items-center gap-2 border-b border-gray-100 dark:border-plum-800 pb-3">
                ...
                <div className="space-y-3">
                  <label className="flex items-start gap-3 p-4 border border-gray-200 dark:border-plum-700 rounded-xl cursor-pointer hover:bg-gray-50 dark:hover:bg-plum-900/50 transition-colors">
                    <input
                      ...
                      className="mt-1 accent-plum-600"
                    />
                    <div>
                      <p className="font-medium text-plum-900 dark:text-ivory-100">
                        Buy & Enquire (Recommended)
                      </p>
                      <p className="text-xs text-plum-500">
                        Customer can either Add to Cart directly or submit an
                        enquiry lead.
                      </p>
                    </div>
                  </label>
                  ...
                  <label className="flex items-start gap-3 p-4 border border-gray-200 dark:border-plum-700 rounded-xl cursor-pointer hover:bg-gray-50 dark:hover:bg-plum-900/50 transition-colors">
                    ...
                    <div>
                      <p className="font-medium text-plum-900 dark:text-ivory-100">
                        Buy Only (Standard E-Commerce)
                      </p>
                      <p className="text-xs text-plum-500">
                        Direct checkout only. No consultation form shown on
                        product page.
                      </p>
                    </div>
                  </label>
                  ...
                  <label className="flex items-start gap-3 p-4 border border-gray-200 dark:border-plum-700 rounded-xl cursor-pointer hover:bg-gray-50 dark:hover:bg-plum-900/50 transition-colors">
                    ...
                    <div>
                      <p className="font-medium text-plum-900 dark:text-ivory-100">
                        Enquire Only (High-Value / Collector Stones)
                      </p>
                      <p className="text-xs text-plum-500">
                        Disables direct cart checkout. Customer must submit an
                        enquiry lead.
                      </p>
                    </div>
                  </label>
                ...
                <div className="pt-4 border-t border-gray-200 dark:border-plum-800 space-y-3">
                  <label className="flex items-center gap-3 p-4 border border-gray-200 dark:border-plum-700 rounded-xl cursor-pointer hover:bg-gray-50 dark:hover:bg-plum-900/50 transition-colors">
                    ...
                    />
                    <span className="text-sm font-medium text-plum-700 dark:text-plum-300">
                      Enable 1-Click WhatsApp Consultation Button on product
                      page
                    </span>
                  </label>
                </div>
