...
export function AddToCart({
  ...
  return (
    <div className="flex items-stretch gap-3">
      {withQuantity && (
        <div className="flex h-14 shrink-0 items-center rounded-none border border-plum-900/15 bg-white">
          <button
            type="button"
            onClick={() => setQty((q) => Math.max(1, q - 1))}
            disabled={qty <= 1}
            aria-label="Decrease quantity"
            className="grid size-12 place-items-center rounded-none text-plum-700 transition-colors hover:bg-ivory-200 disabled:opacity-35"
          >
