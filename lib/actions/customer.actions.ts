...
import { customerAddressSchema, type CustomerAddressInput } from '@/lib/validations/customer.schema';
...
export async function addCustomerAddress(data: CustomerAddressInput) {
  try {
    ...
    if (parsed.data.isDefault || !customer.addresses || customer.addresses.length === 0) {
    ...
    return { success: true, data: JSON.parse(JSON.stringify(customer.addresses)) };
  } catch (error: any) {
...
export async function updateCustomerAddress(addressId: string, data: CustomerAddressInput) {
  try {
    const session = await getSession();
    if (!session?.userId) {
    ...
    if (parsed.data.isDefault) {
      const idx = customer.addresses.findIndex((a: any) => a._id.toString() === addressId);
      if (idx > 0) {
