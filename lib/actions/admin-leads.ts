...
function buildLeadFilter(filters: Record<string, unknown>) {
  ...
  if (typeof filters.status === 'string' && LEAD_STATUSES.includes(filters.status)) {
...
export async function updateLeadStatus(id: string, status: string) {
  try {
    ...
    await dbConnect();
    const lead = await Lead.findByIdAndUpdate(id, { status: parsed.status }, { returnDocument: 'after' });
